...
class TestScrapper(TestCase):
    ...
    @mock.patch('rss.Scrapper.get_rss')
    def test_parse_xml(self,mock_rss):
        ...
        with open(os.path.dirname(os.path.abspath(__file__)) + '/output_result.txt') as f:
            content = f.readlines()
        result = [x.strip() for x in content]

        # I mock the output in a method I'm using to get rss. In this case I'm reading from a file
        mock_rss.return_value = open(os.path.dirname(os.path.abspath(__file__))+ '/../tests/guardian.xml', 'r')
        ...
        self.assertEqual(scraper.list_of_titles, result)
